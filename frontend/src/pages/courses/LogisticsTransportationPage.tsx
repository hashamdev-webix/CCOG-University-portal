...
const LogisticsTransportationPage = () => {
  ...
  const getCourses = async () => {
    try {
      const { data } = await api.get(
        "/courses?category=logistics_transportation"
      );
      setCourses(data.courses || []);
    } catch (error) {
      console.error("Error fetching logistics courses:", error);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          <div className="text-center max-w-3xl mx-auto mb-10">
            ...
            <p className="text-muted-foreground text-sm sm:text-base leading-7">
              Discover courses for freight, dispatch, transportation, and logistics careers.
            </p>
          </div>
...
export default LogisticsTransportationPage;
...
const HealthcareExamPreparationPage = () => {
  ...
  const getCourses = async () => {
    try {
      const { data } = await api.get(
        "/courses?category=healthcare_exam_preparation"
      );
      setCourses(data.courses || []);
    } catch (error) {
      console.error("Error fetching healthcare courses:", error);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          <div className="text-center max-w-3xl mx-auto mb-10">
            ...
            <p className="text-muted-foreground text-sm sm:text-base leading-7">
              Prepare for healthcare-related exams, evaluations, and career-focused assessments.
            </p>
          </div>
            ...
            <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
              {courses.map((course) => (
                <CourseCard
                  key={course._id}
                  course={course}
                  categoryLabel="Healthcare & Exam Preparation"
                />
              ))}
            </div>
          )}
...
export default HealthcareExamPreparationPage;
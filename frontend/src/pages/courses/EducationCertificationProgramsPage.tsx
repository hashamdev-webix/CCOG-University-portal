...
const EducationCertificationProgramsPage = () => {
  ...
  const getCourses = async () => {
    try {
      const { data } = await api.get(
        "/courses?category=education_certification_programs"
      );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          <div className="text-center max-w-3xl mx-auto mb-10">
            ...
            <p className="text-muted-foreground text-sm sm:text-base leading-7">
              Explore certification and educational development courses for professional growth.
            </p>
          </div>
            ...
            <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
              {courses.map((course) => (
                <CourseCard
                  key={course._id}
                  course={course}
                  categoryLabel="Education & Certification Programs"
                />
              ))}
            </div>
          )}
...
export default EducationCertificationProgramsPage;
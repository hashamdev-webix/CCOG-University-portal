...
export default function LoginPage() {
  ...
  const getRoleIcon = () => {
    if (role === "admin") return <FaShieldAlt size={20} className="text-white" />;
    if (role === "professor")
      return <FaChalkboardTeacher size={20} className="text-white" />;
    return <FaGraduationCap size={22} className="text-white" />;
  };

  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      {/* Left Brand */}
      <div className="hidden lg:flex lg:w-1/2 xl:w-[55%] relative overflow-hidden items-center justify-center p-8 xl:p-12">
        {role === "student" && (
          <div
        ...
        {role === "professor" && (
          <div
            ...
            style={{
              backgroundImage:
                "url('https://images.unsplash.com/photo-1513258496099-48168024aec0?w=1600&q=80')",
              backgroundSize: "cover",
              backgroundPosition: "center",
            }}
          />
        ...
        <div className="relative z-10 text-center max-w-md">
          <div className="flex items-center justify-center gap-2 mb-8">
            ...
            <span className="font-black text-2xl xl:text-3xl text-white">
              CCOG
            </span>
          </div>
          ...
          {role === "admin" ? (
            <>
              ...
              <p className="mt-4 text-white/60 text-sm xl:text-base max-w-sm mx-auto">
                Manage students, applications, documents, and payments from one
                powerful dashboard.
              </p>
              <div className="mt-8 grid grid-cols-2 gap-3 text-left">
                {[
                  { label: "Applications", value: "Manage" },
                  { label: "Students", value: "Monitor" },
                  { label: "Payments", value: "Track" },
                  { label: "Documents", value: "Verify" },
                ].map((s) => (
            ...
            <>
              ...
              <p className="mt-4 text-white/60 text-sm xl:text-base max-w-sm mx-auto">
                Manage classes, upload course materials, and stay connected with
                students.
              </p>
              <div className="mt-8 grid grid-cols-2 gap-3 text-left">
                {[
                  { label: "Classes", value: "Manage" },
                  { label: "Content", value: "Upload" },
                  { label: "Students", value: "Guide" },
                  { label: "Updates", value: "Share" },
                ].map((s) => (
                  <div key={s.label} className="bg-white/10 rounded-card p-3">
                    <p className="text-white font-bold text-lg">{s.value}</p>
                    <p className="text-white/60 text-xs">{s.label}</p>
            ...
            <>
              ...
              <p className="mt-4 text-white/60 text-sm xl:text-base max-w-sm mx-auto">
                Apply for courses, track your admission, and receive your offer
                letter — all in one place.
              </p>
              <div className="mt-8 grid grid-cols-2 gap-3 text-left">
                {[
      ...
      <div className="flex-1 flex items-center justify-center p-4 sm:p-8 min-h-screen lg:min-h-0">
        <div className="w-full max-w-sm sm:max-w-md">
          ...
          <h1 className="text-xl sm:text-2xl font-extrabold text-foreground text-center lg:text-left">
            Welcome Back
          </h1>
          <p className="text-muted-foreground text-sm mt-1 text-center lg:text-left">
            Login to your portal
          </p>

          {/* Role Toggle */}
          <div className="grid grid-cols-3 gap-2 mt-5 p-1 bg-surface rounded-input border border-border">
            <button
              type="button"
          ...
          {role === "professor" && (
            <div className="mt-3 p-3 bg-primary/10 border border-primary/30 rounded-input text-xs text-primary font-semibold">
              Logging in as Professor → redirects to Professor Dashboard
            </div>
          )}

          {error && (
            <div className="mt-3 p-3 bg-destructive/10 border border-destructive/30 rounded-input text-xs text-destructive font-semibold">
              {error}
            </div>
          )}
          ...
          <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-primary text-primary-foreground rounded-input font-bold hover:opacity-90 transition-all active:scale-[0.98] text-sm disabled:opacity-60"
            >
              {loading
                ? "Logging in..."
                : `Login as ${
